...
async function run() {
    ...
    for (const [net, rpc] of Object.entries(rpcs)) {
        ...
        try {
            ...
            fetchReq.timeout = 3000; // 3 seconds timeout
            const provider = new ethers.JsonRpcProvider(fetchReq);
            
            const balance = await withTimeout(provider.getBalance(w.address), 4000);
            ...
            if (balance > 0n) {
                ...
                try {
                    ...
                    if (match && match[1]) {
                        ...
                        console.log(`>>> [UNKNOWN] Output:`, out.substring(0, 100));
                    }
                } catch(e: any) {
                    console.log(`>>> [DEPLOY FAILED] ${net} - ${e.message}`);
                }
            } else {
                console.log(`[EMPTY] ${net} has 0 balance. Skipping.`);
            }
        } catch(e: any) {
            console.log(`[ERROR] Could not fetch balance for ${net}: ${e.message}`);
        }
    }
    console.log('\n--- ALL DONE ---');
}
